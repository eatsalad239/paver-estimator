...
const STEP_ORDER = ['service', 'area', 'condition', 'contact'];
const STEP_LABELS = { service: 'Service', area: 'Area', condition: 'Condition', contact: 'Contact' };
...
export function PaverEstimator({ config }) {
  ...
  const handleSubmit = () => {
    ...
    try {
      ...
      const payload = buildLeadPayload({ contact, service, areaLabel, sqft, condition, estimate: est });
  ...
  return (
    <div className="pest-root" style={rootStyle}>
      ...
      {step !== 'result' && (
        <ProgressBar step={stepIndex + 1} total={STEP_ORDER.length} label={STEP_LABELS[step]} />
      )}
